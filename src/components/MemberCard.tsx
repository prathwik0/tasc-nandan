...
import {TwoTwo, TwoThree} from "@/data/Members";
            ...
            <div className={'md:grid md:grid-cols-3 md:gap-6 md:gap-y-14 lg:px-16 md:px-12 px-6 space-y-10 md:space-y-0 pb-28'}>
                {
                    data.map((item, index) => (
                        <>
                            <div key={index} className={' mx-8 md:mx-16 bg-[#f3e7fb] dark:bg-[#272c3a] rounded-lg shadow-xl dark:drop-shadow-md dark:shadow-gray-700  py-8 px-5'}>
                                ...
                                <ul className={'flex justify-center w-full space-x-5 z-50'}>
                                    {item.instagram ? (
                                        <NextLink href={`${item.instagram}`} target={'_blank'}>
